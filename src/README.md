...
-   **Supabase Integration**: Uses Supabase for data storage with tables for `products`, `product_listings`, `external_api_data`, `markets`, and `managers`.
...
3.  **Set up Supabase:**
    ...
    *   Run the following SQL commands to create the necessary tables.
        ...
        -- Managers Table (stores manager information)
        ...
        -- Portfolios Table (placeholder for future portfolio management)
        CREATE TABLE public.portfolios (
            id uuid NOT NULL DEFAULT gen_random_uuid(),
            name TEXT NOT NULL,
            -- Add other portfolio-specific fields here, e.g., description, owner_id, etc.
            created_on TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            modified_on TIMESTAMPTZ DEFAULT NOW(),
            CONSTRAINT portfolios_pkey PRIMARY KEY (id),
            CONSTRAINT portfolios_name_key UNIQUE (name)
        );
        -- Trigger for portfolios.modified_on (similar to markets/managers if needed)
        ...
        CREATE TABLE public.product_listings (
            ...
            created_on TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), -- Add this for listings
            ...
            CONSTRAINT product_listings_manager_id_fkey FOREIGN KEY (manager_id) REFERENCES public.managers (id) ON UPDATE CASCADE ON DELETE RESTRICT, -- Or SET NULL depending on desired behavior
        ...
        -- Trigger for product_listings.updated_at
        CREATE OR REPLACE FUNCTION public.update_product_listings_updated_at_column()
        RETURNS TRIGGER AS $$
        BEGIN
           NEW.updated_at = NOW();
           RETURN NEW;
        ...
        FOR EACH ROW
        EXECUTE FUNCTION public.update_product_listings_updated_at_column();
        ...
        -- Example: Allow read for authenticated users for product data
        ...
        -- Example: Allow CUD for authenticated users (might need to be more specific, e.g., user_id checks)
...
-   Click "Enhance Description" (if available) to use the AI tool (updates `product_listings.data.description`).
...
-   `src/server/services/products.ts`: Service for `products` table operations.
