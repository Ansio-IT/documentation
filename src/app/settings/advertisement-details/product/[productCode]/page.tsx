...
import { RefreshCw, Search, CalendarIcon, XIcon, FileSpreadsheet, ArrowLeft } from 'lucide-react';
...
// Re-using helper functions from the main advertisement-details page
...
const getAmazonBaseUrl = (marketplace?: string | null): string => 'https://www.amazon.co.uk';
...
// Columns Definition (can be the same as the main page, or simplified if needed)
const initialColumnsDefinition: ColumnDef<MarketingDataEntry, any>[] = [
  ...
  // Product Code column no longer needs a link on this page
  ...
  { accessorKey: 'advMarketplace', header: 'Adv Marketplace', size: 90, cell: info => <span className="block truncate" title={String(info.getValue() ?? 'N/A')}>{info.getValue() ?? 'N/A'}</span>, enableSorting: true },
    ...
    if (numericFields.includes(key) && typeof colDef.header === 'string' && !colDef.header.includes('<br />') && typeof colDef.header !== 'function') {
        return { ...colDef, header: () => <div className="text-right w-full">{formatMultiLineHeader(colDef.header as string)}</div>, enableSorting: true };
    }
...
export default function ProductAdvertisementDetailsPage() {
  const params = useParams();
  const router = useRouter();
  const productCodeFromParams = params.productCode as string;
  ...
  const loadMarketingData = useCallback(async () => {
    if (!productCodeFromParams) {
        setIsLoadingReport(false);
        return;
    }
    setIsLoadingReport(true);
    try {
      const fetchParams: FetchMarketingDataParams = {
        pageIndex,
        pageSize,
        ...
        selectedOwner,
        selectedPortfolio,
        productCode: productCodeFromParams,
      };
      const result = await fetchMarketingData(fetchParams);
      setData(result.data || []);
      setPageCount(result.pageCount || 0);
      setTotalDBRowCount(result.totalCount || 0);
    } catch (error) {
      console.error("Error loading marketing data for product:", productCodeFromParams, error);
  ...
  }, [pageIndex, pageSize, sorting, globalFilter, selectedDatePreset, dateRange, selectedOwner, selectedPortfolio, productCodeFromParams, toast]);

  useEffect(() => {
    if (productCodeFromParams) {
      loadMarketingData();
    }
  }, [loadMarketingData, productCodeFromParams]);
    ...
    <TooltipProvider>
      <div className="flex flex-col h-screen p-2 md:p-3">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-semibold text-foreground flex items-center truncate">
            <FileSpreadsheet className="mr-3 h-7 w-7 text-primary shrink-0" />
            <span className="truncate" title={`Advertisement Details for SKU: ${productCodeFromParams}`}>
              Adv. Details for SKU: {productCodeFromParams}
            </span>
          </h1>
          <Button onClick={() => router.back()} variant="outline" size="sm">
            <ArrowLeft className="mr-2 h-4 w-4" /> Back to All Ads
          </Button>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:flex-wrap md:items-end gap-3 pb-3 border-b mb-3">
            <div className="relative min-w-[150px] md:min-w-[200px]">
                ...
                <Input id="adv-global-filter" placeholder="Search within SKU data..." value={globalFilter ?? ''} onChange={(event) => handleGlobalFilterChange(event.target.value)} className="pl-8 w-full h-9 text-sm" />
            </div>
            <div>
            ...
            <div className="flex items-end gap-2 md:ml-auto">
                <RadioGroup value={currentView} onValueChange={(value: 'standard' | 'all') => setCurrentView(value)} className="flex space-x-1 h-9 items-center">
                    <div className="flex items-center space-x-1"><RadioGroupItem value="standard" id="standard-view-prod" className="h-3.5 w-3.5"/><Label htmlFor="standard-view-prod" className="text-xs font-normal cursor-pointer">Standard</Label></div>
                    <div className="flex items-center space-x-1"><RadioGroupItem value="all" id="all-columns-view-prod" className="h-3.5 w-3.5"/><Label htmlFor="all-columns-view-prod" className="text-xs font-normal cursor-pointer">All</Label></div>
                </RadioGroup>
          ...
          <ScrollArea className="w-full rounded-md border bg-card max-h-[500px]">
            <Table className={cn("whitespace-nowrap", currentView === 'standard' ? 'min-w-[1500px]' : 'min-w-[2700px]')}>
              <TableCaption className="py-1 text-xs text-muted-foreground bg-card">
                {(isLoadingReport && (!data || data.length === 0)) ? ( "Loading marketing data..."
                  ...
                  `No marketing data found for SKU: ${productCodeFromParams}.`
                ) : (
                  <>Showing {table.getRowModel().rows?.length ?? 0} of {totalDBRowCount ?? 0} entries for SKU: {productCodeFromParams}. Page {table.getState().pagination.pageIndex + 1} of {table.getPageCount() || 0}</>
                )}
              </TableCaption>
              <TableHeader>
              ...
              <TableBody>
                {isLoadingReport && (!data || data.length === 0) ? (
                  <TableRow><TableCell colSpan={columns.length} className="h-24 text-center whitespace-nowrap"><div className="flex justify-center items-center"><RefreshCw className="h-5 w-5 text-muted-foreground animate-spin mr-2" />Loading data for SKU: {productCodeFromParams}...</div></TableCell></TableRow>
                ) : (!isLoadingReport && data && data.length === 0 && (globalFilter || selectedDatePreset !== 'alltime' || selectedOwner || selectedPortfolio || (dateRange && (dateRange.from || dateRange?.to)))) ? (
                  <TableRow><TableCell colSpan={columns.length} className="h-24 text-center whitespace-nowrap">No results found for your filters for SKU: {productCodeFromParams}.</TableCell></TableRow>
                ) : (!isLoadingReport && (!data || data.length === 0)) ? (
                  <TableRow><TableCell colSpan={columns.length} className="h-24 text-center whitespace-nowrap">No marketing data found for SKU: {productCodeFromParams}.</TableCell></TableRow>
                ) : (
