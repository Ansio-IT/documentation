...
if (!databaseURL || databaseURL === 'YOUR_FIREBASE_DATABASE_URL_HERE' || databaseURL.includes('<YOUR-PROJECT-ID-HERE>') || databaseURL === 'REPLACE_WITH_YOUR_ACTUAL_FIREBASE_DATABASE_URL') {
  throw new Error(
    `Firebase Database URL is not set or is still a placeholder ('${databaseURL}'). Please check your NEXT_PUBLIC_FIREBASE_DATABASE_URL environment variable in .env.local and ensure it's your actual database URL (e.g., https://your-project-id-default-rtdb.firebaseio.com).`
  );
}
if (!projectId) {
  throw new Error(
...
try {
  ...
  const configDatabaseURL = firebaseConfig.databaseURL; 
